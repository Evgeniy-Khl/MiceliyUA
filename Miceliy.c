...
//******************************************************************************
#define MAX_FAZA        7
const char* setFaza[MAX_FAZA]={"Завдання","Динамыка","Дозволи","Управл. А","Управл. D","Просушування","Cпецыальны"}; // ,"Ограничения"
#define MAX_KOFF        8
#ifndef USART
  const char* setKoff[MAX_KOFF]={"Вентилят.притоку","Заслынка повытря","Вентилятор","Кран Гаряч. води","Кран Холод. води","Коэф D12","Коэф D34","Датчики"};
#else
  const char* setKoff[MAX_KOFF]={"Вентилят.притоку","Заслынка повытря","Вентилятор","Кран Гаряч. води","Рециркуляцыя","Коэф D12","Коэф D34","Датчики"};
#endif
#define MAX_FAZA_SET    4
const char* nameFaza[MAX_FAZA_SET+1]={"повытря","компост","вологысть","СО2","ыншы"};
const char* nameD[4]={"Нагрыв","Охолодження","Зволоження","Вентилятор"};
const char* nameE0[3]={"Просушування","Пыдвищення Темп","Зниження Волог"};
const char* nameE1[5]={"Завдання фазы","Стоп системы","Заморожування","Функцыя","ID прибора"};
